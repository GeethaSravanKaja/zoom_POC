...
## *Author*: AAVA
## *Created on*: 
## *Description*: Bronze layer data mapping for Zoom Platform Analytics Systems - updated version excluding API cost information
## *Version*: 2
## *Updated on*: 
_____________________________________________
...
## Overview
This document defines the comprehensive data mapping for the Bronze layer in our Medallion architecture implementation on Snowflake. The Bronze layer serves as the raw data ingestion zone, preserving original data structure while implementing essential metadata management and validation rules.

## Key Changes in Version 2
- **Removed API Cost Information**: Eliminated billing transaction table (bz_billing_transaction) and all API cost-related fields
- **Updated Table Count**: Reduced from 17 tables to 16 tables (15 core business tables + 1 audit table)
- **Enhanced Data Quality Framework**: Improved validation rules and data lineage tracking
- **Optimized for Snowflake**: Enhanced Snowflake-specific optimizations and performance tuning

## Architecture Principles
- **Data Preservation**: Maintain original data structure and format
- **Metadata Enrichment**: Add ingestion timestamps, source tracking, and data lineage
- **Validation Framework**: Implement basic data quality checks without transformation
- **Scalability**: Design for high-volume data ingestion
- **Auditability**: Ensure complete data lineage and change tracking
